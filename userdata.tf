...
sed -i "s@define( 'DB_HOST', 'localhost' )@define( 'DB_HOST', '${element(split(":", aws_db_instance.databasewp.endpoint), 0)}')@g" /var/www/html/wp-config.php
...
# END WordPress
EOT
aws s3 cp --recursive /var/www/html/ s3://capstone-code-bucket
aws s3 sync /var/www/html/ s3://capstone-code-bucket
echo "* * * * * ec2-user /usr/local/bin/aws s3 sync s3://capstone-code-bucket /var/www/html/" > /etc/crontab
echo "* * * * * ec2-user /usr/local/bin/aws s3 sync /var/www/html/wp-content/uploads/ s3://capstone-media-bucket" >> /etc/crontab
